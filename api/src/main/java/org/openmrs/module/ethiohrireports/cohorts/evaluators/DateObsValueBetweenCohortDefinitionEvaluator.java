...
package org.openmrs.module.ethiohrireports.cohorts.evaluators;
...
import org.openmrs.module.ethiohrireports.cohorts.definition.DateObsValueBetweenCohortDefinition;
...
public class DateObsValueBetweenCohortDefinitionEvaluator extends BaseObsCohortDefinitionEvaluator {
	...
	protected Cohort getPatientsHavingObs(BaseObsCohortDefinition cd, RangeComparator operator1, Object value1,
		...
		if (cd.getQuestion() != null) {
			sql.append(" concept_id = :questionConceptId and");
		}
		sql.append(" ((value_coded <> 271) or value_coded is null) ) AS ob1 ");
		...
		if (tm == TimeModifier.ANY || tm == TimeModifier.NO) {
			...
			sql.append("and o.person_id not in (select person_id from obs  where value_coded =271 and voided =false)");
